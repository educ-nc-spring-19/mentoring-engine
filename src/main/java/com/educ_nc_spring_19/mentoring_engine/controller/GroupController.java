...
public class GroupController {
    ...
    public ResponseEntity findById(@PathVariable(name = "id") UUID id) {
        ...
        return group.isPresent()
                ...
                : ResponseEntity.status(HttpStatus.NOT_FOUND).body("Group(id=" + id + ") not found");
    }
    ...
    public ResponseEntity create(@RequestBody GroupDTO groupDTO) {
        ...
        if (StringUtils.isBlank(groupName)) {
            ...
            return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body("'name' body param is empty");
        }
    ...
    public ResponseEntity setBackup(@RequestBody GroupDTO groupDTO) {
        ...
        if (backupId == null) {
            ...
            return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body("'backupId' body param is empty");
        }

        GroupDTO responseGroupDTO;
        try {
            responseGroupDTO = groupMapper.toGroupDTO(groupService.setBackupId(backupId));
        } catch (IllegalArgumentException iAE) {
            log.log(Level.WARN, iAE);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(iAE.getMessage());
        } catch (NoSuchElementException nSEE) {
    ...
    public ResponseEntity addStudentId(@RequestBody StudentDTO studentDTO) {
        ...
        if (studentId == null) {
            log.log(Level.WARN, "studentDTO.id is empty");
            return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body("'id' param is empty");
        }

        GroupDTO responseGroupDTO;
        try {
            responseGroupDTO = groupMapper.toGroupDTO(groupService.addStudentId(studentId));
        } catch (IllegalArgumentException iAE) {
            log.log(Level.WARN, iAE);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(iAE.getMessage());
        } catch (NoSuchElementException nSEE) {
